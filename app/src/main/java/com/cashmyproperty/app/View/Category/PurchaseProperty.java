...
import com.cashmyproperty.app.View.Response.PurchaseAdapter_Recycler;
import com.cashmyproperty.app.View.Response.PurchasePropertyDatum;
...
public class PurchaseProperty extends Fragment {
    RecyclerView rec_purchase;
    MaterialToolbar tool_bar;
    PurchaseAdapter_Recycler purchaseAdapter_recycler;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        rec_purchase = view.findViewById(R.id.rec_purchase);

        tool_bar=view.findViewById(R.id.tool_bar);
        tool_bar.setNavigationIcon(R.drawable.ic_baseline_keyboard_backspace_24);

        rec_purchase.setLayoutManager(new LinearLayoutManager(getActivity()));
    ...
    private void getpurchaseData() {
        ...
        dataViewModel.getpurchaseproperty(getActivity()).observe(getActivity(), new Observer<Result>() {
            @Override
            public void onChanged(Result result) {
                //hideLoader();
                if (result != null) {

                        List<PurchasePropertyDatum> purchasePropertyData=result.getPurchasePropertyData();

                        if (purchasePropertyData.size()>0) {
                            showprogressbar(false);
                            purchaseAdapter_recycler = new PurchaseAdapter_Recycler(getActivity(),purchasePropertyData);
                            rec_purchase.setAdapter(purchaseAdapter_recycler);
